...
def get_admin_main_menu(queue_count: int = 0, payout_count: int = 0, is_super_admin: bool = False) -> InlineKeyboardMarkup:
    ...
    Args:
        ...
        is_super_admin: Если True, показывает кнопки управления выплатами, админами, бонусами и логами.
    """
    ...
    # 2. Функции, доступные ТОЛЬКО Супер-Админу
    if is_super_admin:
        ...
        builder.row(InlineKeyboardButton(text="📜 Скачать логи (.csv)", callback_data="download_logs"))
    
    # 3. Общие кнопки (Статистика и Управление пользователями)
    builder.row(
...
def get_user_management_keyboard(db_user_id: int, is_banned: bool, can_manage: bool) -> InlineKeyboardMarkup:
    ...
    # Кнопки действий показываем только если у админа есть права
    if can_manage:
        if is_banned:
            builder.row(InlineKeyboardButton(
                text="✅ Разблокировать", 
