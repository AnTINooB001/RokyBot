...
def get_admin_main_menu(queue_count: int = 0, payout_count: int = 0) -> InlineKeyboardMarkup:
    ...
    builder.row(
        InlineKeyboardButton(text="📊 Статистика", callback_data="show_stats_menu"), # Изменен callback
...
def get_admin_cancel_keyboard() -> InlineKeyboardMarkup:
    ...
    builder = InlineKeyboardBuilder()
    builder.row(InlineKeyboardButton(text="❌ Отмена", callback_data="admin_cancel_action"))
    return builder.as_markup()